...
class Job extends BaseJob {
  ...
  fork(thunk, options) {
    if (this.forkEnabled()) {
      ...
      const pool = workerpool.pool(Path.resolve(__dirname, 'workers/forking-workerpool.js'), opts);
...
class RecurringJob extends BaseJob {
  ...
  /**
   * Update the job's priority, retries, interval, data or klass.
   */
  update(config) {
    // When setting 'data', the provided value must be JSON serialized
    const serialized = Object.assign({}, config, {
      data: JSON.stringify(config.data || this.data),
    });

    return Promise.each(Object.keys(serialized), (key) => {
      const value = serialized[key];

      return this.client.call('recur.update', this.jid, key, value);
    }).then(() => {
  ...
  next() {
    return this.client.redis.zscoreAsync(`ql:q:${this.queueName}-recur`, this.jid);
  }

  move(queue) {
    return this.client.call('recur.update', this.jid, 'queue', queue);
  }
  ...
  tag() {
    return this.client.call.apply(
      this.client, ['recur.tag', this.jid].concat(Array.from(arguments)));
  }

  untag() {
    return this.client.call.apply(
      this.client, ['recur.untag', this.jid].concat(Array.from(arguments)));
  }
}
